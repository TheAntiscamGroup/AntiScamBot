...
from discord import Webhook, ui, TextStyle, Interaction, Member, User
from Logger import Logger, LogLevel
from TextWrapper import TextLibrary
from typing import TYPE_CHECKING, override

if TYPE_CHECKING:
  type InputBox = ui.TextInput[ui.view.BaseView]
  from Types import *

class ScamReportData():
  ReportingUserName: str = ""
  ReportingUserId: int = -1
  ReportedServer: str = ""
  ReportedServerId: int|None = None
  ReportedUserGlobalName: str = ""
  ReportedUserName: str = ""
  ReportedUserId: int = -1
  TypeOfScam: str = ""
  Reasoning: str = ""
  Evidence: list[str]|None = None
  ReportWebhook: Webhook|None = None

  def __getitem__(self, item: str) -> Any:  # pyright: ignore[reportAny, reportExplicitAny]
...
class SubmitScamReport(ui.Modal):
  ...
  TypeOfScam: InputBox = ui.TextInput(label=Messages["report","type","label"], required=True,
                    placeholder=Messages["report","type","msg"], max_length=50, min_length=10)
  Reasoning: InputBox = ui.TextInput(label=Messages["report","details","label"],
                    placeholder=Messages["report","details","msg"],
                    style=TextStyle.paragraph,
                    max_length=700, required=False)
  ScamEvidence: InputBox = ui.TextInput(label=Messages["report","evidence","label"],
  ...
  async def on_submit(self, interaction: Interaction) -> None:
    Bot: DiscordBot = interaction.client
    ...
    ScamReportPayload: ScamReportData = {
      "ReportingUserName": interaction.user.name,
      "ReportingUserId": interaction.user.id,
      "ReportedServer": interaction.guild.name if interaction.guild is not None else "",
      ...
      "ReportWebhook": interaction.followup
    }
